...
// // Icons (EditIcon, TrashIcon, etc.) stay same — omitted for brevity
// const EditIcon = () => (
//   <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
//     <path strokeLinecap="round" strokeLinejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
//   </svg>
...
//   const flatten = (arr) => {
//     arr.forEach((field) => {
//       if (field.name) bundle[field.name] = field.value;
...
// const shouldDisplayField = (item, formData) => {
//   const jsCondition = item.dynamic?.visibility?.conditions?.dynamic;

//   if (jsCondition) {
//     try {
//       const fn = new Function("formData", "bundle", `return (${jsCondition})`);
//       return fn(formData, formData);
//     } catch (e) {
//       console.error("JS Condition Error:", jsCondition, e);
//       return false;
//     }
//   }
...
// const Render = memo(({ data, deleteField, addfield, increase, dicrease, onEditField, path = "", fullFormData }) => {
//   const bundle = buildBundleFromArray(fullFormData);
...
// it's fully working 
// import React, { memo, useCallback, useMemo } from 'react'; // Import useMemo
// import { motion, AnimatePresence } from 'framer-motion';
// import ApiRender from './ApiRender';
...
//     arr.forEach((field) => {
//       if (field.name) {
//         bundle[field.name] = field.value; // Assign the field's value to its name in the bundle
//       }
//       if (Array.isArray(field.children)) flatten(field.children);
//     });
//   };
...
// const shouldDisplayField = (item, bundle) => { // Still takes bundle as direct argument
...
//   onEditField,
//   path = "",
//   fullFormData // fullFormData is still passed down
// }) => {
//   // Use useMemo to create the bundle.
//   // It will only re-run buildBundleFromArray if fullFormData changes.
...
//         );
//     }
//   }, []); // Dependencies are stable or related to item, not fullFormData/bundle
...
//           // shouldDisplayField now uses the memoized bundle
//           if (!shouldDisplayField(item, bundle)) return null;
...
//                   onEditField={onEditField}
//                   path={`${currentPath}.children`}
//                   fullFormData={fullFormData} // fullFormData continues to be passed down recursively
//                 />
//               )}
...
// import { motion, AnimatePresence } from 'framer-motion';
// // ApiRender (assuming it's defined elsewhere or as a simple placeholder)
// const ApiRender = ({ field }) => {
//   return (
//     <div className="text-xs text-blue-600 bg-blue-100 p-2 rounded-md mt-2">
//       API Source: {field.source?.type || 'N/A'}
//     </div>
//   );
// };


// Form field components (assuming these are imported from their respective files)
// For a complete runnable example, these would be defined like in the previous App.js block.
...
//   path = "",
//   fullFormData,
//   onFieldChange // Passed down from DynamicFormBuilder
// }) => {
//   const bundle = useMemo(() => buildBundleFromArray(fullFormData), [fullFormData]);

//   // renderFormField now calculates dynamic options directly
//   const renderFormField = useCallback((item, currentPath) => {
//     let fieldOptions = item.options || []; // Default to static options
...
//       try {
//         // Evaluate the jsExpression directly here
...
//         fieldOptions = Array.isArray(result) ? result : [];
        
//       } catch (e) {
//         console.error("JS Options Expression Error for field:", item.name, "Expression:", item.dynamic.options.jsExpression, "Error:", e);
//         console.error("Current bundle state:", bundle);
//         fieldOptions = [{ label: "Error loading options", value: "" }]; // Show an error option
//       }
//     }

//     const commonProps = {
//       item: { ...item, options: fieldOptions }, // Override item.options with dynamic ones if applicable
//       currentPath,
//       onFieldChange // Pass the change handler to the field components
//     };
...
//         );
//     }
//   }, [onFieldChange, bundle]); // Add bundle to dependencies, as it's used directly here
...
//           // Use the memoized bundle for display conditions
//           if (!shouldDisplayField(item, bundle)) return null;
